##' compute_cellSTAAR_pvalue_multlink.
...
##' @export compute_cellSTAAR_pvalue_multlink

compute_cellSTAAR_pvalue_multlink<-function(data_obj,grouping_vars=c("gene","chr","phenotype","element_class","ct_name"),use_conditional_p_if_exist=FALSE){
  ...
  t1<-data_obj%>%group_by(across(all_of(grouping_vars)))%>%mutate(CCT_pval=CCT_removeNA(.data$pvalue),avg_num_rare_var=round(mean(num_rare_var,na.rm=TRUE),0))%>%dplyr::select(all_of(grouping_vars),CCT_pval,avg_num_rare_var)%>%distinct()%>%ungroup()
