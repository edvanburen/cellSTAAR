...
##' @export create_cellSTAAR_mapping_file_multlink
create_cellSTAAR_mapping_file_multlink<-function(gds.path
                                        ,sc_epi_file_path
                                        ,ct_name
  ...
  for(link_type in link_types_to_run){
      ...
      if(element_class%in%c("pELS","dELS")){
        if(cCRE_version=="V3"){
          all_dist<-bind_rows(cellSTAAR::raw_mappings_cCRE_V3_dist_0_1%>%
                              ...
                              ,cellSTAAR::raw_mappings_cCRE_V3_dist_1_50000%>%
                                filter(chr==paste0("chr",!!chr))
                              ,cellSTAAR::raw_mappings_cCRE_V3_dist_50000_100000%>%
                                filter(chr==paste0("chr",!!chr))
                              ,cellSTAAR::raw_mappings_cCRE_V3_dist_100000_150000%>%
                                filter(chr==paste0("chr",!!chr))
                              ,cellSTAAR::raw_mappings_cCRE_V3_dist_150000_200000%>%
                                filter(chr==paste0("chr",!!chr))
                              ,cellSTAAR::raw_mappings_cCRE_V3_dist_200000_250000%>%
                                filter(chr==paste0("chr",!!chr)))
        }
        if(cCRE_version=="V4"){
          all_dist<-bind_rows(cellSTAAR::raw_mappings_cCRE_V4_dist_0_1%>%
                                filter(chr==paste0("chr",!!chr))
                              ,cellSTAAR::raw_mappings_cCRE_V4_dist_1_50000%>%
                                filter(chr==paste0("chr",!!chr))
                              ,cellSTAAR::raw_mappings_cCRE_V4_dist_50000_100000%>%
                                filter(chr==paste0("chr",!!chr))
                              ,cellSTAAR::raw_mappings_cCRE_V4_dist_100000_150000%>%
                                filter(chr==paste0("chr",!!chr))
                              ,cellSTAAR::raw_mappings_cCRE_V4_dist_150000_200000%>%
                                filter(chr==paste0("chr",!!chr))
                              ,cellSTAAR::raw_mappings_cCRE_V4_dist_200000_250000%>%
                                filter(chr==paste0("chr",!!chr)))
        }
      }
      raw_mappings_dist<-all_dist%>%filter(chr==paste0("chr",!!chr))
      raw_mappings_dist<-raw_mappings_dist%>%mutate(gene_dist_all=coalesce(.data$gene_dist_0_1,.data$gene_dist_1_50000,.data$gene_dist_50000_100000,.data$gene_dist_100000_150000,.data$gene_dist_150000_200000,.data$gene_dist_200000_250000))%>%dplyr::select(.data$chr,.data$start,.data$end,.data$width,.data$cCRE_accession,.data$classification1,.data$classification2,.data$gene_dist_all)
    ...
    if(link_type=="nondist_link_all"){
      if(element_class%in%c("pELS","dELS")){
        ...
        if(cCRE_version=="V4"){
          all_map<-bind_rows(cellSTAAR::raw_mappings_cCRE_V4_ABC_link_all_50
                             %>%filter(chr==paste0("chr",!!chr))
                             ,cellSTAAR::raw_mappings_cCRE_V4_EpiMap_link_all_50%>%filter(chr==paste0("chr",!!chr))
                             ,cellSTAAR::agnostic_dnase_summary_V4_eQTL%>%filter(chr==paste0("chr",!!chr))
                             ,cellSTAAR::agnostic_dnase_summary_V4_noneQTL%>%filter(chr==paste0("chr",!!chr)))
        }
      }
      all_map<-all_map%>%filter(chr==paste0("chr",!!chr))
      all_map<-all_map%>%mutate(gene_nondist_all=coalesce(.data$ABC_gene,.data$EpiMap_gene,.data$gene))%>%dplyr::select(.data$chr,.data$start,.data$end,.data$width,.data$cCRE_accession,.data$classification1,.data$classification2,.data$gene_nondist_all)
    ...
    bp_level_mappings_dist<-function(obj,filt,other_cts=FALSE){
      ...
      dist_val<-gsub("_dist_link","",dist_val)
