...
-   If a user has limited computational resources or is only interested in certain genes (for example, to replicate an exernal analysis), the <code>genes_manual</code> parameter in the <code>run_cellSTAAR</code> function allows users to specify specific genes to analyze. 
